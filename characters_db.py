# characters_db.py
from models import General
import random
import maps_db 
import equipment_db

# ==========================================
#   第一部分：語言中樞 (Dialogue System)
# ==========================================

# 1. VIP 專屬台詞庫 (Tier 0: Legends)
vip_dialogues = {
    "曹操": [
        "寧教我負天下人，休教天下人負我！",
        "對酒當歌，人生幾何？譬如朝露，去日苦多。",
        "周公吐哺，天下歸心。",
        "孤好夢中殺人，汝等切勿近前。",
        "今天下英雄，唯使君與操耳！",
        "設使國家無有孤，不知當幾人稱帝，幾人稱王？"
    ],
    "劉備": [
        "勿以惡小而為之，勿以善小而不為。",
        "漢室傾頹，奸臣竊命，備不量力，欲伸大義於天下。",
        "兄弟如手足，妻子如衣服。",
        "備欲求賢若渴，公可教我？",
        "惟賢惟德，能服於人。",
        "二弟、三弟，若能在同年同月同日死，亦無憾矣。"
    ],
    "孫權": [
        "生子當如孫仲謀！",
        "內事不決問張昭，外事不決問周瑜。",
        "孤承父兄基業，必當開疆拓土。",
        "曹孟德到了？再探！",
        "這荊州，我看是借不還了。",
        "老夫親射虎，看孫郎。"
    ],
    "諸葛亮": [
        "鞠躬盡瘁，死而後已。",
        "非淡泊無以明志，非寧靜無以致遠。",
        "主公之志，亮願效犬馬之勞。",
        "我從未見過如此厚顏無恥之人！",
        "今當遠離，臨表涕零，不知所云。",
        "亮夜觀天象，知天下三分已定。"
    ],
    "關羽": [
        "吾觀顏良，如插標賣首耳！",
        "玉可碎而不可改其白，竹可焚而不可毀其節。",
        "關某的大刀，早已飢渴難耐。",
        "虎女焉能嫁犬子！",
        "酒且斟下，某去便來。",
        "土雞瓦狗，不堪一擊！"
    ],
    "張飛": [
        "燕人張翼德在此！誰敢決一死戰？",
        "俺也一樣！",
        "三姓家奴休走！",
        "大戰三百回合，怕你不成！",
        "大哥，這鳥位讓俺也坐坐！",
        "把那督郵捆起來打！"
    ],
    "周瑜": [
        "既生瑜，何生亮！",
        "丈夫處世兮立功名，立功名兮慰平生。",
        "談笑間，檣櫓灰飛煙滅。",
        "我得為都督分憂才是。",
        "大江東去，浪淘盡，千古風流人物。"
    ],
    "呂布": [
        "大丈夫生居天地間，豈能鬱鬱久居人下！",
        "人中呂布，馬中赤兔。",
        "吾被酒色所傷，竟如此憔悴...",
        "看我方天畫戟！",
        "父親！",
        "誰敢擋我！"
    ],
    "司馬懿": [
        "我揮劍只有一次，可我磨劍磨了十幾年。",
        "人這一生，不光活個生死，還活個對錯。",
        "諸葛村夫，你中計了！",
        "權且忍讓。",
        "老夫這就送你上路。",
        "善敗者不亡。"
    ],
    "卑彌呼": [
        "日出之處的天子，致書日沒之處的天子...",
        "鬼道之力，汝無法理解。",
        "這片海域，皆在我的掌控之中。",
        "神諭已下，汝無路可逃。",
        "呵呵...有趣的異鄉人。",
        "邪馬台的風，今日有些喧囂。"
    ],
    "趙雲": [
        "吾乃常山趙子龍也！",
        "主公放心，雲視百萬曹軍如草芥！",
        "龍膽亮銀槍在此！",
        "陷陣之志，有死無生！",
        "子龍一身都是膽！"
    ],
    "貂蟬": [
        "大人，妾身舞姿如何？",
        "妾身願為大人分憂。",
        "亂世浮萍，身不由己。",
        "將軍，請自重。",
    ],
    "華佗": [
        "救人一命，勝造七級浮屠。",
        "將軍此去，兇多吉少。",
        "頭痛？來，待老夫為你開顱...",
        "麻沸散一服，便無痛楚。"
    ]
}

# 2. 原型台詞庫 (Tier 1: Archetypes)
# 用於填充沒有專屬台詞的武將

archetype_dialogues = {
    "warrior": [ # 猛將型 (武力 > 80)
        "拳頭才是硬道理！", "看什麼看？想打架嗎？", "我的大斧已經飢渴難耐了。", 
        "亂世之中，強者為尊。", "在下每日聞雞起舞，從未懈怠。", "比武切磋？奉陪到底！",
        "戰場上只有生死，沒有對錯。", "取汝首級，如探囊取物！", "吾之武藝，天下無雙！"
    ],
    "strategist": [ # 謀士型 (智力 > 80)
        "兵者，詭道也。", "多算勝，少算不勝。", "天下大勢，分久必合，合久必分。",
        "在下喜讀兵書，略懂一二。", "蠻力是解決不了問題的。", "且慢，容我三思。",
        "運籌帷幄之中，決勝千里之外。", "此計甚妙，主公可依計行事。", "觀天象，今夜有雨。"
    ],
    "official": [ # 官員/統帥型 (統御 > 80)
        "治大國如烹小鮮。", "軍紀嚴明，方能百戰不殆。", "百姓安居樂業，方是正道。",
        "食君之祿，擔君之憂。", "這座城的防務還需要加強。", "公務繁忙，恕不遠送。",
        "令行禁止，違者斬！"
    ],
    "foreign": [ # 外族/異域型 (位於特定地點)
        "中原的酒，不夠烈。", "草原上的雄鷹，從不畏懼風暴。", "長生天在看著我們。",
        "你們漢人，規矩太多。", "要買馬嗎？上好的戰馬。", "為了部落！"
    ],
    "common": [ # 普通人/雜魚
        "這世道，活著不容易啊。", "聽說米價又漲了。", "不知道家鄉的梅花開了沒。",
        "你是哪裡來的？看著面生。", "最近風聲緊，小心點。", "在下只是一介武夫。",
        "想喝一杯嗎？", "今日天氣不錯。", "唉，何時才能太平..."
    ]
}

def assign_dialogues(general):
    """
    智慧型台詞分配系統
    """
    # 1. 優先分配 VIP 台詞
    if general.name in vip_dialogues:
        general.dialogues = vip_dialogues[general.name]
        return

    # 2. 根據屬性與地點分配原型台詞
    pool = []
    
    # 地點檢測 (外族優先)
    # 假設 ID 30(敦煌), 42(邪馬台), 60(大漠), 99(秦嶺), 102(遼東) 為外族區
    if general.location_id in [30, 42, 60, 99, 102]:
        pool += archetype_dialogues["foreign"]

    # 屬性檢測
    if general.war >= 80:
        pool += archetype_dialogues["warrior"]
    if general.int_ >= 80:
        pool += archetype_dialogues["strategist"]
    if general.ldr >= 80:
        pool += archetype_dialogues["official"]
    
    # 兜底
    if not pool:
        pool = archetype_dialogues["common"]
    
    # 3. 從池中隨機抽取 3-5 句，形成該武將的獨特語錄
    count = min(len(pool), random.randint(3, 5))
    general.dialogues = random.sample(pool, count)


# ==========================================
#   第二部分：歷史數據 (Historical Data)
# ==========================================
# 擴充至 150+ 人

historical_data = [
    # === 魏 (曹操軍團) ===
    ("曹操", 88, 96, 99, 1), ("司馬懿", 68, 99, 97, 1), ("荀彧", 25, 98, 95, 1),
    ("郭嘉", 20, 99, 70, 1), ("賈詡", 30, 98, 85, 7), ("程昱", 40, 92, 70, 4),
    ("夏侯惇", 92, 60, 85, 2), ("夏侯淵", 93, 55, 88, 7), ("曹仁", 88, 70, 92, 17),
    ("曹洪", 82, 50, 75, 2), ("張遼", 94, 80, 95, 6), ("張郃", 91, 72, 88, 7),
    ("徐晃", 92, 70, 85, 1), ("于禁", 85, 75, 88, 1), ("樂進", 86, 50, 78, 1),
    ("典韋", 98, 30, 50, 1), ("許褚", 97, 35, 60, 2), ("龐德", 95, 60, 80, 7),
    ("李典", 78, 75, 72, 5), ("滿寵", 60, 85, 88, 12), ("鄧艾", 88, 95, 92, 7),
    ("鍾會", 60, 94, 90, 7), ("羊祜", 70, 90, 95, 15), ("杜預", 70, 85, 90, 17),
    ("甄宓", 30, 85, 70, 6), ("卞氏", 20, 75, 80, 1), ("曹丕", 75, 85, 80, 1),
    ("曹植", 30, 92, 40, 1), ("楊修", 25, 93, 30, 1), ("華歆", 30, 85, 70, 1),
    ("王朗", 35, 80, 70, 1), ("陳群", 25, 90, 88, 1), ("劉曄", 30, 92, 60, 5),
    ("郝昭", 85, 88, 90, 7), ("郭淮", 80, 85, 85, 7), ("曹真", 75, 70, 85, 7),

    # === 蜀 (劉備軍團) ===
    ("劉備", 75, 78, 90, 18), ("諸葛亮", 40, 100, 99, 18), ("龐統", 35, 98, 85, 18),
    ("法正", 30, 96, 80, 18), ("徐庶", 50, 95, 80, 15), ("關羽", 97, 75, 88, 17),
    ("張飛", 99, 35, 70, 18), ("趙雲", 96, 78, 85, 19), ("馬超", 97, 45, 80, 30),
    ("黃忠", 93, 65, 80, 16), ("魏延", 92, 50, 80, 19), ("姜維", 90, 92, 90, 7),
    ("關平", 85, 65, 75, 17), ("周倉", 86, 30, 60, 17), ("關興", 84, 60, 70, 18),
    ("張苞", 85, 40, 65, 18), ("馬岱", 86, 50, 75, 19), ("王平", 80, 75, 85, 19),
    ("廖化", 78, 65, 75, 17), ("嚴顏", 82, 60, 78, 18), ("黃月英", 30, 95, 60, 18),
    ("孫尚香", 85, 70, 60, 11), ("孟獲", 85, 20, 70, 99), ("祝融", 88, 40, 65, 99),
    ("孟優", 75, 20, 50, 99), ("木鹿大王", 80, 10, 60, 99), ("兀突骨", 90, 5, 40, 99),
    ("蔣琬", 30, 90, 88, 18), ("費禕", 30, 88, 85, 18), ("馬良", 25, 88, 70, 17),
    ("馬謖", 60, 85, 60, 18), ("李嚴", 70, 75, 80, 18), ("劉禪", 10, 10, 50, 18),

    # === 吳 (孫家軍團) ===
    ("孫權", 70, 82, 95, 11), ("孫策", 95, 70, 92, 12), ("孫堅", 92, 75, 90, 16),
    ("周瑜", 75, 97, 98, 11), ("魯肅", 40, 95, 92, 11), ("呂蒙", 80, 90, 92, 17),
    ("陸遜", 70, 96, 95, 17), ("甘寧", 94, 60, 85, 16), ("太史慈", 95, 65, 82, 14),
    ("周泰", 92, 30, 60, 11), ("凌統", 88, 50, 70, 14), ("程普", 82, 70, 85, 16),
    ("黃蓋", 83, 65, 80, 104), ("韓當", 80, 60, 75, 103), ("徐盛", 82, 75, 85, 11),
    ("丁奉", 84, 60, 80, 11), ("朱然", 75, 70, 85, 17), ("諸葛謹", 40, 88, 85, 11),
    ("張昭", 20, 90, 85, 11), ("張紘", 25, 88, 80, 11), ("顧雍", 30, 85, 80, 14),
    ("大喬", 20, 85, 60, 12), ("小喬", 20, 85, 60, 11), ("步練師", 25, 80, 70, 11),
    ("全琮", 75, 75, 80, 50), ("賀齊", 80, 70, 85, 50), ("潘璋", 80, 40, 70, 17),
    ("蔣欽", 82, 55, 75, 14), ("董襲", 85, 40, 60, 14),

    # === 袁紹/河北 ===
    ("袁紹", 70, 70, 85, 6), ("顏良", 95, 30, 70, 6), ("文醜", 96, 25, 65, 6),
    ("田豐", 20, 94, 80, 6), ("沮授", 30, 92, 85, 6), ("高覽", 86, 55, 70, 8),
    ("審配", 50, 85, 80, 6), ("逢紀", 30, 82, 60, 6), ("郭圖", 35, 75, 50, 8),
    ("袁術", 60, 50, 40, 12), ("紀靈", 85, 40, 70, 12), ("淳于瓊", 75, 40, 60, 2),

    # === 荊州/西涼/群雄 ===
    ("劉表", 40, 75, 65, 15), ("蔡貌", 70, 70, 80, 15), ("張允", 65, 60, 75, 15),
    ("黃祖", 70, 60, 70, 16), ("文聘", 85, 70, 85, 15), ("蒯良", 20, 85, 70, 15),
    ("韓遂", 75, 70, 80, 30), ("馬騰", 85, 50, 80, 30), ("華雄", 92, 30, 60, 3),
    ("呂布", 100, 30, 80, 4), ("貂蟬", 20, 80, 60, 3), ("陳宮", 40, 90, 80, 4),
    ("高順", 88, 50, 85, 4), ("呂玲綺", 88, 60, 65, 4),
    ("董卓", 88, 50, 80, 3), ("李儒", 20, 93, 70, 3),
    ("公孫瓚", 82, 60, 75, 13), ("公孫度", 75, 65, 75, 13),
    ("張角", 60, 85, 95, 6), ("張寶", 70, 75, 80, 6), ("張梁", 80, 60, 75, 6),
    ("左慈", 10, 99, 10, 99), ("于吉", 10, 98, 10, 50), ("華佗", 20, 98, 20, 12),
    ("司馬徽", 10, 95, 20, 15),

    # === 異族與海外 ===
    ("卑彌呼", 50, 98, 95, 42), ("難升米", 60, 70, 65, 42), ("壹與", 30, 90, 85, 42),
    ("蹋頓", 85, 40, 75, 60), ("軻比能", 88, 55, 80, 102), ("步度根", 85, 50, 75, 60),
    ("呼廚泉", 80, 45, 70, 60), ("蔡文姬", 15, 92, 50, 60),
    ("衛溫", 60, 60, 70, 50), ("諸葛直", 55, 65, 60, 50),
]

# ==========================================
#   第三部分：生成與模擬工廠
# ==========================================

surnames = "趙錢孫李周吳鄭王馮陳褚衛蔣沈韓楊朱秦尤許何呂施張孔曹嚴華金魏陶姜"
names_1 = "伯仲叔季文武忠孝仁義禮智信德昭明輝光顯達震驚天動海山雲風雷電神鬼龍虎"
names_2 = "一二三四五六七八九十元亨利貞乾坤陰陽"

def generate_mass_generals(target_count=350):
    current_list = []
    
    # 1. 實例化史實名將
    for data in historical_data:
        name, war, int_, ldr, loc = data
        gen = General(name, war, int_, ldr, location_id=loc)
        
        gen.gold = random.randint(1200, 3000)
        gen.level = random.randint(15, 35)
        assign_dialogues(gen)

        # === [新增] 逸品分發邏輯 ===
        # 搜尋所有 owner_name 屬於該武將的逸品
        my_artifacts = [
            e for e in equipment_db.all_artifacts 
            if e.owner_name == name
        ]
        
        for art in my_artifacts:
            # 強制裝備 (無視部位衝突，史實優先)
            # 因為 equip 方法會把舊的換下來，我們直接調用
            gen.equip(art)
            # 確保逸品不在背包而在裝備欄，且不會被隨機覆蓋
            # (equip 方法已經處理了屬性加成)
        # =========================
        
        current_list.append(gen)
    
    # ... (後續補充隨機武將的代碼保持不變) ...
        
    # 2. 補充大眾臉武將
    needed = target_count - len(current_list)
    if needed < 0: needed = 0
    valid_locs = list(maps_db.cities.keys())
    
    for _ in range(needed):
        name = random.choice(surnames) + random.choice(names_1) + random.choice(names_2)
        
        # 數值常態分佈
        war = int(random.gauss(45, 12)); war = max(10, min(85, war))
        int_ = int(random.gauss(45, 12)); int_ = max(10, min(85, int_))
        ldr = int(random.gauss(45, 12)); ldr = max(10, min(85, ldr))
        loc = random.choice(valid_locs)
        
        gen = General(name, war, int_, ldr, location_id=loc)
        gen.gold = random.randint(100, 800)
        gen.level = random.randint(1, 15) # 起始等級較低
        
        # [關鍵] 分配台詞 (會使用原型庫)
        assign_dialogues(gen)
        
        current_list.append(gen)
        
    return current_list

# 初始化武將池
all_generals = generate_mass_generals(360)

def simulate_world_turn():
    """
    世界回合同步：NPC 移動與成長
    """
    move_log = []
    
    for gen in all_generals:
        if gen.name == "軒轅無名": continue
            
        # 1. 移動邏輯 (15% 機率)
        if random.random() < 0.15:
            current_city = maps_db.cities.get(gen.location_id)
            if current_city and current_city.get('connections'):
                next_loc = random.choice(current_city['connections'])
                gen.location_id = next_loc
                
                # 名將跨區移動才記錄
                if (gen.get_total_stat('war') > 85 or gen.get_total_stat('int_') > 85) and \
                   current_city.get('region') != maps_db.cities[next_loc].get('region'):
                     move_log.append(f"傳聞：{gen.name} 離開{current_city['region']}，前往 {maps_db.cities[next_loc]['name']}。")

        # 2. 成長邏輯 (10% 機率)
        if random.random() < 0.1:
            growth = random.choice(['war', 'int_', 'ldr'])
            gen.grow(growth, 1)
            # 隨機獲取少量經驗
            gen.gain_xp(random.randint(10, 50))
            
    return move_log
